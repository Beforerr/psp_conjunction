...
function workload()
    psp_config = Dict(:id => "PSP", :B => PSP.B, :tau => 30)
    wind_config = Dict(:id => "Wind", :B => Wind.B_GSE, :tau => 30)
    thm_config = Dict(:id => "THEMIS", :B => THEMIS.B_FGL_GSE, :tau => 30)
    ...
    df = mapreduce(vcat, 7:9) do enc
        ...
        mapreduce(vcat, configs) do c
            ...
            res, = produce_or_load(c, datadir(); tag = false) do c
                c[:events] = ids_finder(c[:B], c[:t0], c[:t1], Second(c[:tau]))
                c
            end
