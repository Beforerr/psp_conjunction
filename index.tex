...
\begin{tcolorbox}[enhanced jigsaw, colback=white, opacitybacktitle=0.6, opacityback=0, leftrule=.75mm, toprule=.15mm, left=2mm, colframe=quarto-callout-note-color-frame, breakable, colbacktitle=quarto-callout-note-color!10!white, toptitle=1mm, titlerule=0mm, bottomtitle=1mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, arc=.35mm, rightrule=.15mm, bottomrule=.15mm, coltitle=black]

This document and the accompanying website may contain outdated information.
...
The fifth and sixth panels show the median Alfvén ratio, \(R_{VB} = |Δ 𝐕| / |Δ 𝐕_A|\), of the current sheets and the total number of current sheets identified within each four-hour window. Including the anisotropy factor \(Λ = μ(p_{\parallel,e} − p_{\perp,e} + p_{\parallel,i} − p_{\perp,i})/B^2\) does not significantly affect the Alfvén ratio \(R_{VB}^* = R_{VB} / \sqrt{1 - Λ}\); its average value is approximately 0.5 for PSP and 0.4 for Wind over the entire time interval. The number of current sheets varies from 100 to 300 per four-hour window, corresponding to an average of about 1,000 and 900 current sheets per day for PSP and Wind, respectively. The PSP occurrence rates are broadly consistent with previous studies at similar heliocentric distances \citep{lotekarKineticscaleCurrentSheets2022}. We detect significantly more current sheets than the ∼150 events reported at 1 AU by \citet{vaskoKineticscaleCurrentSheets2022, zhangSolarWindDiscontinuities2025b}, largely because our detection is not restricted to only the most intense kinetic-scale current sheets (see methods below).
Panels (a.7) and (b.7) present the cross helicity and residual energy, calculated every 8 minutes, which characterize the Alfvénic nature and energetic balance of the ambient plasma. We compute these quantities in the standard form: \(\sigma_c = \frac{2 \langle \delta \mathbf{V} \cdot \delta \mathbf{V}_A \rangle}{ \langle\delta\mathbf{V}^2\rangle + \langle\delta\mathbf{V}_A^2\rangle }, \quad \sigma_r = \frac{\langle\delta \mathbf{V}^2\rangle-\langle\delta \mathbf{V}_A^2\rangle}{\langle\delta \mathbf{V}^2\rangle+\langle\delta \mathbf{V}_A^2\rangle},\) where \(\delta \mathbf{V}\) and \(\delta \mathbf{V}_A=\delta( \mathbf{B} / \sqrt{\mu_0 \rho})\) are the fluctuating velocity and Alfvén velocity, respectively.
...
\pandocbounded{\includegraphics[keepaspectratio]{figures/Q_sonnerup_joint_dist_den.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{figures/joint_properties-cross.png}}
