...
tvars_psp = [
    ...
    set(tnorm_combine ∘ PSP.V; ylabel=V_ylabel, labels=V_RTN_labels),
...
```{julia}
tvars_wind = [
    set(tnorm_combine ∘ Wind.B_RTN_1MIN; ylabel=B_ylabel, labels=B_RTN_labels),
    set(Wind.n; ylabel=n_ylabel),
    set(Wind.V_RTN; ylabel=V_ylabel, labels=V_RTN_labels),
...
data = THEMIS.V_GSE(tmin_wind, tmax_wind)
_times = @views SPEDAS.times(data)[(data[:, 1].>-200u"km/s").|(data[:, 2].<-50u"km/s")]
dt = Minute(10)
...
```{julia}
tvars_themis = [
    set(_tmask! ∘ tnorm_combine ∘ THEMIS.B_GSE; ylabel=B_ylabel, labels=B_XYZ_labels),
    THEMIS.n,
    _tmask! ∘ THEMIS.V_GSE,
    set(THEMIS.T; ylabel=T_ylabel)
]
...
tplot(f[1, 1], tvars_themis, tmin_wind, tmax_wind; legend)
add_labels!(f[1, 1]; labels, open="(c.")
f
# easy_save("overview")
```
