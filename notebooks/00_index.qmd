...
tselect(A, t) = A[Ti(Near(t))]
function tselect(A, t, δt)
    tmp = @views A[Ti((t - δt) .. (t + δt))]
    length(tmp) == 0 ? missing : tmp[Ti(Near(t))]
end
...
function get_vl_ratio_ts(df)
    vl_ratio_ts1 = DimArray(df.v_l_ratio, Ti(df.time))
    vl_ratio_ts2 = DimArray(df.v_l_ratio_Λ, Ti(df.time))
    gfunc = x -> floor(x, Hour(4))
    vl_ratio_ts = hcat(
        nanmedian.(disallowmissing.(groupby(vl_ratio_ts1, Ti => gfunc))),
        nanmedian.(disallowmissing.(filter.(!ismissing, groupby(vl_ratio_ts2, Ti => gfunc))))
    )
    set_meta!(vl_ratio_ts; ylabel=L"R_{VB}", labels=["Isotropy", "Anisotropy"])
    return vl_ratio_ts
end
```
...
function modify_faxs_vl!(faxs)
    ylims!(faxs.axes[end-1], 0.09, 0.85)
    faxs.axes[4].ylabel[] = "T (eV)"
    β_ax = faxs.axes[6]
    β_ax.yscale[] = log10
    β_ax.ylabel[] = "β"
    ylims!(β_ax, 8e-2, 4e1)
...
for enc in 7:9
    ...
    if plot_vl_ratio
        ...
        n = PSP.n_spi(tmin_psp, tmax_psp)
        T = PSP.pTemp(tmin_psp, tmax_psp; add_unit=false)
        ...
        psp_v_l_ratio_ts = get_vl_ratio_ts(psp_df)
        tvars_psp = (tnorm_combine(B), n, V, [T, Tp, Te], psp_v_l_ratio_ts, β)


        Wind_Tp2 = Wind.pTemp_T2(tmin_wind, tmax_wind)
        Wind_Te2 = Wind.eTemp_T2(tmin_wind, tmax_wind)
        ...
        T = rebuild(Wind.T_p_PLSP(tmin_wind, tmax_wind) |> tsort; metadata=(; labels=[L"T_p"]))
        B_mag = tnorm(B)
        β = Discontinuity.plasma_beta.(tsync(T, n, B_mag)...)

        wind_v_l_ratio_ts = get_vl_ratio_ts(wind_df)
        tvars_wind = (tvars_wind..., [T, Wind.Temp_ani[1:2]...], wind_v_l_ratio_ts, β)
    end
