...
using NCDatasets
using YAXArrays
path = "data/psp_e07.nc"
# ds = NCDataset(path)
ds = YAXArrays.open_dataset(path)
```
...
psp_V_mva = Product("mva_V", (psp_V, psp_B), mva_transform; ylabel=V_ylabel, labels=mva_V_labels)

event = psp_events[2, :]
plot_candidate(event, [tvars_psp; [psp_B_mva, psp_V_mva]])
```
