...
fs = map([:mva]) do 𝐧
    ...
    vars = [𝒎.l_log, 𝒎.l_norm_log, 𝒎.j_log, 𝒎.j_norm_log, 𝒎.Δ𝐁, 𝒎.Δ𝐁_norm_log]
    labels = ["Thickness", "Normalized thickness", "Current density", "Normalized current density", "|Δ𝐁|", "|Δ𝐁|/⟨|𝐁|⟩"]
    # append!(vars, [ :Δ𝐁_fit => log10 => "Log Δ𝐁_fit", :Δ𝐁_fit_norm => log10 => "Log Δ𝐁_fit_norm"])
    # append!(labels, ["Δ𝐁_fit", "Δ𝐁_fit_norm"])

    plt = AoG.data(tdf) *
          mapping(vars, layout=AoG.dims(1) => renamer(labels)) *
          mapping(color=:id, linestyle=:enc) *
          AoG.density(; datalimits) * visual(Lines)
    f = draw(plt, scales(Layout=(; palette=wrapped(cols=2))); axis=(; yscale=log10), figure, legend=(; position=:top))
    hide_facet_labels!(f)
...
for 𝐧 in [:mva]
    ...
    B_l_spec = plt * mapping(𝒎.Δ𝐁_norm, 𝒎.l_norm) # Not strong correlation
    B_j_spec = plt * mapping(𝒎.Δ𝐁_norm, 𝒎.j_norm) # Not strong correlation

    grid1 = draw!(fig[1, 1], spec1; axis)
    grid2 = draw!(fig[2, 1], spec2; axis)
    grid3 = draw!(fig[3, 1], B_l_spec; axis)
    grid4 = draw!(fig[4, 1], B_j_spec; axis)

    hide_facet_labels!.((grid2, grid3, grid4))

    add_labels!(fig[1:4, 1]; position=Left(), padding=(0, 37, 0, 0))
    display(fig)
    easy_save("joint_properties-$𝐧"; formats, force=true)
end
```

### In-plane rotation angle, Bn

![](../figures/B_n_ω_subset=true.png) and ![](../figures/B_n_ω_subset=false.png)
...
for 𝐧 in [:mva], subset in (true, false)
    ...
    easy_save("B_n_ω_subset=$subset"; formats, force=true)
end
```
...
No obvious correlation for PSP and Wind ![](../figures/β_Q_sonnerup.png).
