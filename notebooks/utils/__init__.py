...
def calc_pressure_anisotropy(df: pl.DataFrame):
    ion_temp_para = df["ion_temp_para"].to_numpy() * u.eV
    ion_temp_perp = df["ion_temp_perp"].to_numpy() * u.eV
    e_temp_para = df["e_temp_para"].to_numpy() * u.eV
    e_temp_perp = df["e_temp_perp"].to_numpy() * u.eV

    B = (df["B.before"].to_numpy() + df["B.after"].to_numpy()) / 2 * u.nT
    n = df["plasma_density"].to_numpy() * u.cm**-3
    ...
    return df.with_columns(Λ_ion=Λ_ion, Λ_e=Λ_e, Λ=Λ)
